...
theorem: for every real number~$a$ and every $N\in\N$, the $N$ points
$\{ia \bmod 1 : 0 \le i < N\}$ partition the half-open interval $[0,1)$ into
...
indices realizing it would differ by a period bounded strictly between the two
return times, contradicting their minimality.  This removes the irrationality
...
corner such a period would lie strictly between the two return indices,
contradicting their minimality.  For irrational~$a$ no nonzero period exists and
...
strictly between the two return indices.  We derive a contradiction from the
...
period of the orbit exceeds $\mpl+\mmi$ in the corner, so no period can sit in
...
\hline
Orbit $\orb(a,N)$, count $k$        & \code{orbit}, \code{orbitCard}            & 53, 72\\
Sorted points $x_i$                 & \code{sortedVal}                          & 84\\
Gap multiset; $\code{gapAt}$        & \code{gaps}, \code{gapAt}                 & 95, 157\\
Gaps sum to $1$ (Lem.~\ref{lem:sum-one}) & \code{gaps\_sum\_eq\_one}            & 105\\
Reduction (Lem.~\ref{lem:reduction})& \code{three\_gap\_card\_le\_three\_of\_subset} & 189\\
Returns $\etap,\etam$ (Def.~\ref{def:returns}) & \code{etaPos}, \code{etaNeg}   & 246, 251\\
Return minimality (Lem.~\ref{lem:return-min}) & \code{no\_orbit\_below\_etaPos}, & 273,\\
                                    & \quad\code{le\_one\_sub\_etaNeg}          & 291\\
Canonical index                     & \code{canMul}                             & 474\\
Return indices $\mpl,\mmi$          & \code{mPlus}, \code{mMinus}               & 578, 770\\
Shift identity (Lem.~\ref{lem:shift})& \code{fract\_index\_shift}               & 650\\
No point between (Lem.~\ref{lem:npb})& \code{noPointBetween}, \code{noPointBetween\_neg} & 697, 810\\
Case A (Prop.~\ref{prop:caseA})     & \code{gap\_eq\_etaPos}                     & 735\\
Case B (Prop.~\ref{prop:caseB})     & \code{gap\_eq\_etaNeg}                     & 852\\
Discriminant (Lem.~\ref{lem:hP1})   & \code{corner\_canMul\_lt\_mMinus}         & 1103\\
Returns preserved (Lem.~\ref{lem:returns-preserved}) & \code{etaPos\_eq\_extend}, & 1058,\\
                                    & \quad\code{etaNeg\_eq\_extend}            & 1075\\
L1, L3 (Lem.~\ref{lem:L1L3})        & \code{corner\_noPoint\_lo}, \code{corner\_noPoint\_hi} & 1174, 1224\\
Corner gap; L2 (Prop.~\ref{prop:corner}, Thm.~\ref{thm:L2}) & \code{corner\_gap} & 1271\\
Classification                      & \code{neighbour\_gap\_in\_returns}        & 1427\\
Main theorem (Thm.~\ref{thm:main})  & \code{three\_gap\_card\_le\_three}        & 1460\\
Sum relation (\S\ref{subsec:example} text) & \code{three\_gap\_lengths\_eq}     & 1474\\
\hline
\end{tabular}
...
slices of the same elementary geometry, and both are formalized in the same
repository.
