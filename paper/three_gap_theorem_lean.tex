...
\hypersetup{
  pdftitle={A Lean 4 Formalization of the Three-Gap (Steinhaus) Theorem,
    Uniform in the Rotation Number},
  pdfauthor={Dirk Kunert},
  pdfkeywords={three-gap theorem, three-distance theorem, Steinhaus conjecture,
    Lean 4 formalization, Mathlib, machine-checked proof},
}
...
via \code{Int.fract} rather than \code{AddCircle}.  Having searched Mathlib,
the Archive of Formal Proofs, the Coq/Rocq contribution archive, Metamath,
HOL~Light, HOL4, Mizar and the Lean Zulip archive, we have found no earlier
machine-checked formalization of the theorem valid for an arbitrary
\emph{real} rotation number: the prior formal proofs we found---Mayero's Coq
development after van Ravenstein, and two independent Lean~4 developments of
2026, one of them proposed to Mathlib but closed unmerged---all carry an
irrationality hypothesis, and the theorem remains absent from Mathlib at the
time of writing.  The geometric crux, that the longest gap contains no orbit
point, is discharged in the corner case by interval arguments together with a
self-contained return-index/period argument for the critical midpoint: were
the midpoint an orbit point, the two
indices realizing it would differ by a period strictly greater than the
positive return index and strictly less than the sum of the two return indices,
...
\code{three\_gap\_card\_le\_three} and the three-values identification
\code{three\_gap\_lengths\_eq} (which yields the sum relation) are proved with
no \code{sorry}, depending only on
...
of the theorem: the uniform statement is classical, and the rational case has
been treated explicitly in the literature.  Our contribution is the
...
(see~\cite{BertheReutenauer2024} for a recent account); we recall the proof
...
point $\etap$ (the \emph{positive return}) and $\etam$, the gap from
...
proof assumes $a$ irrational, so that distinct indices always give distinct
points; the rational case, where the orbit is periodic and indices collide,
is set aside.  The prior formalizations we found all inherit such an
irrationality hypothesis, and in each of them it is load-bearing rather than
decorative.  In Mayero's Coq development~\cite{Mayero2000,MayeroCoq1998},
which follows van Ravenstein, the section containing the main theorem opens
with the hypothesis $\alpha p \ne n$ for all integers $n,p$, and that
hypothesis is threaded through the development some seventy-six times.  In the
Lean~4 pull request proposed to Mathlib but closed
unmerged~\cite{SaarShai2026} it sits already in the \emph{definition} of the
gap function, so that the uniform statement cannot even be phrased there.  We close
...
We make a deliberately narrow claim, and we state it as what it is: a search
result, not a theorem about the literature.  Having searched Mathlib master
and the mathlib4 pull requests, the Archive of Formal Proofs, the Coq/Rocq
contribution archive, Metamath, HOL~Light, HOL4, Mizar and the Lean Zulip
archive (all as of $4$~August~$2026$), we have found no earlier
machine-checked formalization of the three-gap theorem valid for an arbitrary
\emph{real} rotation number, that is, without an irrationality hypothesis
on~$a$ or an equivalent restriction.\footnote{The qualification
``real'' is deliberate.  A machine-checked three-gap statement without an
irrationality hypothesis does exist, but only as a decidable instance on a
fixed finite cyclic group: a Lean file in an unrelated music-theory
repository~\cite{MusicMath2026} bounds the number of distinct step sizes on
$\Z/12\Z$ by three, proved by exhausting the twelve generators.  It
quantifies over no real~$a$, covers no irrational rotation number at all, and
the file containing it was committed on $27$~June~$2026$, after the first
commit of the present development ($24$~June~$2026$).  We note that the
repository itself was created two days before ours, so that a comparison of
repository dates alone would be misleading.}  Every earlier formalization we
found carries such a hypothesis.  We do \emph{not} claim priority for the
formalization as such: the Mathlib pull request~\cite{SaarShai2026} precedes
this development by four weeks.  We do \emph{not}
claim a new proof of the theorem itself; elementary proofs valid for
all~$a$ have long been available, and the rational case has been treated
explicitly in the literature~\cite{AlessandriBerthe1998}.  We
survey the proof landscape, including the dynamical and geometric proofs, in
Section~\ref{sec:related}.  The minimality of the two returns is, moreover,
...
them, and Section~\ref{sec:discussion} discusses a Mathlib
contribution---in part already merged---and further directions.
...
order embedding \code{Finset.orderEmbOfFin} on the first $k$ indices (and set
to a junk value $0$ beyond).  We write
...
For the empty orbit ($N=0$, so $k=0$) the description above does not apply;
there the Lean definition degenerates, via the total-function convention for
\code{sortedVal}, to the junk singleton multiset $\{1\}$, and all statements
below hold trivially.  This is the only place the junk value of
\code{sortedVal} surfaces.
...
\begin{lemma}[Reduction, \code{three\_gap\_card\_le\_three\_of\_subset}]\label{lem:reduction}
...
in the set $\{\etap,\,\etam,\,\etap+\etam\}$ (of at most three elements).  The degenerate
...
least index $\ell<N$ with $\fract{\ell\,a}=p$ (the minimum of the nonempty
\emph{fibre} $\{\ell<N : \fract{\ell a}=p\}$).  The \emph{return indices} are the
...
=\fract{x+y}$
(\code{fract\_add\_fract\_eq})\footnote{Since the merge of mathlib4 pull
request~\#42080~\cite{KunertMathlib2026} ($31$~August~$2026$) this identity
and its subtraction companion \code{fract\_sub\_fract\_eq} are available in
Mathlib as \code{Int.fract\_fract\_add\_fract} and its \code{sub} analogue;
...
\begin{proof}[Proof idea]
By Lemma~\ref{lem:npb} nothing lies in $(v,v+\etap)$, so in particular
$v+\etap\le v'$; since $v'<1$, this gives $v+\etap<1$ strictly.  (The lemma
\code{fract\_add\_etaPos\_le\_one} yields only $v+\etap\le 1$, and equality is
attained in general; the strict bound comes from the successor $v'$.)  By
Lemma~\ref{lem:shift} (no wrap) the point $v+\etap=\fract{(j+\mpl)a}$ is then an
orbit point above $v$, and by \code{no\_orbit\_strictly\_between} nothing lies
in $(v,v')$; the two facts force $v'=v+\etap$.
\end{proof}
...
$q\le v'<1$, so $q<1$.  Only now is $q$ realized without wrap:
...
\begin{proof}[Proof idea]
No new index $w\in[N,M)$ can produce a point $s=\fract{w a}$ in $(0,\etap)$:
subtracting each return index in turn puts two \emph{old} orbit points around
$s$---the index $w-\mmi<\mpl\le N$ realizes $s+\etam$, so minimality gives
$\etap\le s+\etam$, and the index $w-\mpl<\mmi\le N$ realizes $1-(\etap-s)$ in
the top band, so $\etam\le\etap-s$.  Together these squeeze $s+\etam=\etap$
exactly; but then $w-\mmi$ realizes $\etap$, so by canonicity $\mpl\le w-\mmi$,
i.e.\ $w\ge\mpl+\mmi=M$, contrary to $w<M$ (\code{no\_new\_below\_etaPos}).
The top band $(1-\etam,1)$ is excluded by the mirror argument
(\code{no\_new\_above\_etaNeg}).  Hence the minimum positive point and the
maximum are the same in $\orb(a,M)$ as in $\orb(a,N)$.
\end{proof}

Because $j+\mpl<M$, the forward ``no point between'' lemma now \emph{does} apply
at $v$ inside $\orb(a,M)$, giving L1.  For L3 the same in-range realization
$v+\etap=\fract{(j+\mpl)a}$ in $\orb(a,M)$ is used directly: an orbit point in
$(v+\etap,\,q)$ would, via the difference of its index and $j+\mpl$, place a
point of $\orb(a,M)$ in the top band $(1-\etam,1)$, contradicting return
minimality there (the returns of $\orb(a,M)$ coincide with $\etap,\etam$ by
Lemma~\ref{lem:returns-preserved}).  Both conclusions are stated for the
original orbit since $\orb(a,N)\subseteq\orb(a,M)$.
...
\begin{proof}
Suppose $v+\etap=\fract{w\,a}$ with $w<N$.  Recall
$v+\etap=\fract{(j+\mpl)a}$, with $j+\mpl\ge N$ by the corner condition.

\emph{Case $w\ge j$.}  Then $\fract{(w-j)a}=\fract{w a}-\fract{j a}=\etap$ is an
orbit point realizing $\etap$ at index $w-j<N$, so by minimality of $\mpl$ we
have $\mpl\le w-j$, i.e.\ $w\ge j+\mpl\ge N$---contradicting $w<N$.  (This is
exactly where the corner condition $j+\mpl\ge N$ is used.)

\emph{Case $w<j$.}  Now $\fract{w a}=v+\etap=\fract{(j+\mpl)a}$ with
$w\ne j+\mpl$, so their difference is an integer multiple of~$a$ with vanishing
fractional part:
\[
  d := j+\mpl-w, \qquad \fract{d\,a}=0, \qquad \mpl<d<\mpl+\mmi .
\]
(The bounds are $d>\mpl$ from $w<j$, and $d\le j+\mpl<\mpl+\mmi$ from
...
\end{itemize}
Either way the case $w<j$ is impossible, and $v+\etap\notin\orb(a,N)$.
\end{proof}
...
force $a=(\text{integer})/d\in\Q$.  So the case $w<j$ is vacuous, and L2 reduces
to the elementary case~$w\ge j$---this is the situation treated by van
Ravenstein and formalized by Mayero.  For rational $a$ the period is genuine,
and the bound $\mpl<d<\mpl+\mmi$ is precisely what rules it out: the argument
shows that no period of the rotation lies in the open window $(\mpl,\mpl+\mmi)$,
which is exactly what the collision would require.  The single argument above
covers both regimes, which is what lets the Lean proof avoid any
rational/irrational case split; and it proceeds without induction---no descent
along continued fractions is needed, which is what the title of this
...
\subsection{Assembling the theorem}\label{subsec:assemble}
...
visible numerically---the minimal period of the rotation here is $7$, while
$\mpl+\mmi=5$, so
there is no period $d$ at all in the window $(\mpl,\mpl+\mmi)=(3,5)$, and the
collision that case~$w<j$ would require cannot occur.
...
recovers the index structure (\code{mulFiber}, \code{canMul}, $\mpl$) and
proves the fractional-part shift identities.  Cases A and B occupy the forward
and backward ``index-bridge'' blocks, in which $\mmi$ is also set up; they end
in \code{gap\_eq\_etaPos} and \code{gap\_eq\_etaNeg}.  The final block is the
corner: the discriminant
...
Sum relation (\S\ref{subsec:assemble} text) & \code{three\_gap\_lengths\_eq}    & 1475\\
\hline
\end{tabular}
...
\code{\#print axioms ThreeGap.three\_gap\_card\_le\_three} reports
\[
...
``rigid gap'' proof of the $3d$ distance theorem, of which the three-gap
theorem is the case $d=1$.  We claim no new mathematics and, in particular, no
new proof: the uniform statement is classical, and the rational case has been
treated explicitly in the literature---S\'os and Slater both address it, as
recorded in the survey of Alessandri and Berth\'e~\cite{AlessandriBerthe1998}.
Uniformity is thus not by itself a new feature of any proof, and we
...
higher-dimensional generalizations.  A recent exposition gives a concise
geometric proof through Farey pairs and mediants~\cite{Geometric2023}.
Berth\'e and Reutenauer~\cite{BertheReutenauer2024} revisit the theorem from
the word-combinatorial viewpoint, focusing on the finite words that encode the
successive gap lengths.

\emph{Formalization.}  Every machine-checked treatment of the theorem we have
been able to find states it only for irrational rotation numbers.  The
earliest is Mayero's~\cite{Mayero2000}, carried out in Coq (now Rocq) after
van Ravenstein.  There the hypothesis is explicit in the source of the
development itself~\cite{MayeroCoq1998}: the section containing the main
theorem \code{three\_gap} opens by assuming $\alpha p \ne n$ for all
integers $n,p$, and the assumption recurs some seventy-six times across the
four files.  Mayero is candid about the restriction, omitting the rational
case because ``most of the intermediate results are false for alpha
rational,'' and closing with the remark that ``in our proof it should be
supposed that alpha is rational or not and we do not see, so far how to avoid
this distinction.''  In 2026 two independent Lean~4 developments of the
irrational case appeared.  In the first, proposed to Mathlib as pull
request \#40037~\cite{SaarShai2026} and closed unmerged on
contribution-policy rather than mathematical grounds, the hypothesis is
structural rather than incidental: the gap function itself carries it, so
that the uniform statement cannot be phrased there at all.  In the
second~\cite{LeanGenius2026} the gap definitions are hypothesis-free, but all
three main theorems still assume that~$a$ is irrational; the file also
records that its build verification was still pending.  Neither proves a
counterpart of the sum relation \code{three\_gap\_lengths\_eq}.  As of
$4$~August~$2026$ the theorem is absent from Mathlib---whose \code{Steinhaus}
lemmas concern the Banach--Steinhaus theorem and the measure-theoretic
Steinhaus theorem, not this one---and the Archive of Formal Proofs contains
no Isabelle formalization; searches of Metamath, HOL~Light, HOL4, Mizar, the
Coq/Rocq contribution archive and the Lean Zulip archive turned up nothing
further.  We therefore know of no earlier machine-checked statement of the
theorem for an arbitrary real rotation number.  Its place in the landscape is
...
into a library contribution (subject to Mathlib's contribution
guidelines).  Two adaptations would help: phrasing the statement
...
For the \code{Int.fract} half this has meanwhile happened: the absorption
lemmas behind the identity $\fract{\fract{x}+\fract{y}}=\fract{x+y}$ of
Section~\ref{sec:cases} were contributed to Mathlib as pull
request~\#42080~\cite{KunertMathlib2026}, merged into Mathlib master on
$31$~August~$2026$.  Mathlib now carries the four one-sided \code{@[simp]}
lemmas \code{Int.fract\_fract\_add}, \code{Int.fract\_add\_fract},
\code{Int.fract\_fract\_sub}, \code{Int.fract\_sub\_fract} and the two-sided
corollaries \code{Int.fract\_fract\_add\_fract} and
\code{Int.fract\_fract\_sub\_fract} (in
\code{Mathlib.Algebra.Order.Floor.Ring}), stated for any linearly ordered
ring with a floor structure; they generalize this module's
\code{fract\_add\_fract\_eq} and \code{fract\_sub\_fract\_eq} from~$\R$.
The module itself still compiles against its pinned pre-merge Mathlib
version and therefore retains its local proofs.

\emph{Relation to the companion work.}  The orbit $\orb(a,N)$ studied here is,
for rational $a$, the same kind of projected point set as those produced by the
one-dimensional rational cut-and-project construction whose gap sequence and
period---under both the multiset and set conventions---are the subject of a
companion paper~\cite{Kunert2026}.  The three-gap theorem bounds the number
of distinct gap \emph{lengths} appearing in a single window; the companion work
...
repository (\cite{Kunert2026}, \cite{KunertThreeGap2026}).
...
text.  The one-sided formulation of the \code{Int.fract} absorption lemmas that
were upstreamed to Mathlib (Section~\ref{sec:discussion}) follows a suggestion
by Eric Wieser.
...
\bibitem{AlessandriBerthe1998}
Pascal Alessandri and Val\'{e}rie Berth\'{e},
``Three distance theorems and combinatorics on words,''
\emph{Enseign.\ Math.}, series~2, vol.~44, pp.~103--132, 1998.
...
preprint, arXiv:\href{https://arxiv.org/abs/2308.11999}{2308.11999}, 2023.

\bibitem{Kunert2026}
Dirk Kunert,
``Period Length Formulas for Rational Cut-and-Project Strip Projections:
Multiset and Set Cases,'' with Lean~4 formalization,
\emph{Repository cut-and-project}, version \texttt{v1.5-arxiv}, 2026.\newline
Available at: \url{https://github.com/dkunert/cut-and-project/tree/v1.5-arxiv}.

\bibitem{KunertMathlib2026}
Dirk Kunert,
``feat(Algebra/Order/Floor): add fract-absorption lemmas for sums and
differences,''
pull request \#42080, \emph{leanprover-community/mathlib4}, opened 25~July
2026, merged 31~August 2026.\newline
Available at:
\url{https://github.com/leanprover-community/mathlib4/pull/42080}.

\bibitem{KunertThreeGap2026}
Dirk Kunert,
``A Lean~4 Formalization of the Three-Gap (Steinhaus) Theorem, Uniform in the
Rotation Number,''
\emph{Repository three-gap-theorem-lean}, 2026.\newline
Available at: \url{https://github.com/dkunert/three-gap-theorem-lean}.

\bibitem{LeanGenius2026}
Robert Walters and contributors,
``\code{Erdos998ThreeGapOQ04.lean},'' in
\emph{Repository lean-genius}, June~2026.\newline
Available at: \url{https://github.com/rjwalters/lean-genius}.
...
\bibitem{MayeroCoq1998}
Micaela Mayero,
``ThreeGap: a proof of the three gap theorem (Steinhaus conjecture),''
Coq contribution, INRIA Rocquencourt, September~1998; archived at
\url{https://github.com/coq-contribs/three-gap} (file \code{three\_gap.v}).
...
\bibitem{MusicMath2026}
Carles Mar\'{\i}n Mu\~{n}oz,
``\code{DiatonicScale.lean},''
\emph{Repository music-math}, commit of $27$~June~2026.\newline
Available at:
\url{https://github.com/karlesmarin/music-math/blob/main/lean/DiatonicScale.lean}.

\bibitem{SaarShai2026}
Saar Shai,
``feat(NumberTheory): the three-gap (Steinhaus) theorem,''
pull request \#40037, \emph{leanprover-community/mathlib4}, opened 30~May
2026, closed unmerged 9~June 2026.\newline
Available at:
\url{https://github.com/leanprover-community/mathlib4/pull/40037}.
